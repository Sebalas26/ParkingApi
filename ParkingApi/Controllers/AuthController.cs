        ...
        if (!string.IsNullOrEmpty(sidClaim) && int.TryParse(sidClaim, out int userId))
        {
            ...
            if (profile != null && profile.Success)
            {
                return Ok(new
                {
                    ...
                    hasDesktopAccess = profile.HasDesktopAccess,
                    hasWebAccess = profile.HasWebAccess,
                    allowMultipleSessions = profile.AllowMultipleSessions,
                    maxActiveSessionsPerUser = profile.MaxActiveSessionsPerUser,
                    allowMultipleOpenShifts = profile.AllowMultipleOpenShifts,
                    maxOpenShiftsPerUser = profile.MaxOpenShiftsPerUser,
                    requireOpenShiftToOperate = profile.RequireOpenShiftToOperate,
                    requireInitialCashAmount = profile.RequireInitialCashAmount,
